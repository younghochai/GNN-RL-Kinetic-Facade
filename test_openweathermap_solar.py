"""
OpenWeatherMap Solar Irradiance API 통합 테스트 스크립트
...
2. 실행:
   python test_openweathermap_solar.py

또는 runtime.yaml에 API 키 설정 후:
   python test_openweathermap_solar.py --use-config
"""
...
from src.rl.runtime.weather import fetch_weather_data, fetch_openweathermap_solar_irradiance
...
def test_fetch_weather_data(cfg: dict) -> None:
    """기상 데이터 조회 테스트."""
    print("\n" + "=" * 80)
    print("TEST 1: fetch_weather_data() - OpenWeatherMap Solar API 호출")
    print("=" * 80)

    ts_kst = pd.Timestamp.now(tz=pytz.timezone("Asia/Seoul"))
    print(f"조회 시각 (KST): {ts_kst.strftime('%Y-%m-%d %H:%M:%S')}")

    # 태양 위치 계산 (선택, Solar API는 필요 없음)
...
def test_fetch_solar_irradiance(cfg: dict) -> None:
    """DNI/DHI 조회 테스트."""
    print("\n" + "=" * 80)
    print("TEST 2: fetch_openweathermap_solar_irradiance() - DNI/DHI 직접 조회")
    print("=" * 80)
    ...
    try:
        irradiance = fetch_openweathermap_solar_irradiance(ts_kst, cfg)
...
def main():
    parser = argparse.ArgumentParser(description="OpenWeatherMap Solar API 통합 테스트")
    parser.add_argument(
        "--use-config",
        action="store_true",
    ...
    # Provider 설정
    cfg["weather_provider"] = "openweathermap_solar"
    
    # 기상 데이터 소스 설정
    if "openweathermap_solar" in cfg:
        cfg["openweathermap_solar"]["weather_source"] = args.weather_source
        print(f"기상 데이터 소스: {args.weather_source}")

    # API 키 확인
    api_key = os.environ.get("OPENWEATHERMAP_API_KEY", "")
    if not api_key and not args.use_config:
        print("❌ 오류: OPENWEATHERMAP_API_KEY 환경변수가 설정되지 않았습니다.")
        print("\n해결 방법:")
        print("  1. export OPENWEATHERMAP_API_KEY='your_api_key'")
        print("  2. python test_openweathermap_solar.py --use-config (runtime.yaml 사용)")
        sys.exit(1)
    ...
    try:
        ...
        if not args.skip_irradiance:
            test_fetch_solar_irradiance(cfg)
        else:
            print("\n⏭️  DNI/DHI 테스트 건너뛰기")
        ...
        print("=" * 80)
        print("\n다음 단계:")
        print("  1. runtime.yaml에서 weather_provider를 'openweathermap_solar'로 설정")
        print("  2. policy.ipynb 또는 run.ipynb에서 실행")
        print("  3. 캐시 TTL 조정 (필요 시)")
        print("\n⚠️  주의:")
        print("  - OpenWeatherMap Solar API는 Pay-as-you-call (유료)")
        print("  - 캐싱(5분 TTL)으로 API 호출 최소화")
        print("  - Clear Sky vs Cloudy Sky 모델 선택 가능")
